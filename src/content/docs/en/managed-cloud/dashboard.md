...
description: How to use your Managed Cloud account — projects, domains, billing, and account.
---

The cloud dashboard is the management interface for your account and your projects — separate
from the actual [legal text admin](/en/features/) of each individual project, which you switch
to via single sign-on (see [SSO handoff](#switching-to-legal-text-management) below).

## Login & overview

After logging in you see your current plan (plan chip), your project usage (e.g. "1/1
projects"), and a status badge per project (**active**, **provisioning**, **DNS pending**,
**SSL pending**, or **suspended**). New projects also show a "getting started" checklist. If a
...
As long as your plan limit isn't reached, you can create another project via "New project" — a
...
(Flex: multiple projects, tiered pricing from 2 projects). Switching happens with one click plus
confirmation, billed pro-rata via Stripe. Downgrading to Solo is only possible once just one
project remains. Enterprise terms are arranged individually — contact support for that.
...
method, invoices, and cancellation. After canceling, your project stays active until the end of
...
[Paragrafy admin](/en/features/) of that project — no additional login required. From there you
manage the compliance matrix, translations, webhooks, and every other core feature.

## Deleting a project

A project can be deleted at any time via the corresponding button (with a confirmation prompt
and a reminder to download a backup first). Deletion is blocked if it's your last project or if
a custom domain is still attached — remove the domain first.

### Deletion protection

The lock icon next to each project enables deletion protection. While it's active, the delete
button is hidden and the project cannot be deleted by accident — editing, viewing, and every
other action are unaffected. Clicking the lock again lifts the protection. Handy for actively used
projects where an accidental click would be costly.
